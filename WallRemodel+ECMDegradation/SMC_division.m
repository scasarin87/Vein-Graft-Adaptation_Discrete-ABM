...
sigma_norm=4e1; % normalization constant for division in medial layer
...
% Wall Shear Stress
tau_norm=4^-1; % normalization constant for division in intimal layer
...
%% Probability Laws
test=rand(1); % reference probability
...
% Pdivision
if lamina(j,k)<=0, %inside the intima, regulated by shear stress
    p_division=probability_SMC(2)*activity*(1+A(6)*Delta_tau_wall); 
else % in the media, regulated by wall tension
