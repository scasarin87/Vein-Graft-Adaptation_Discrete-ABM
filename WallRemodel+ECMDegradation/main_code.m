...
epsilon = 0.5; % percentile perturbation to generate target shear stress
xsi = 0.005; % percentile perturbation to generate target wall tension
...
% basic solution/follow up time (= 8 months simulated)
basic_sol = 2 * months_days*days_hours; % 2 months of basic sol generation
simulation_time = 6 * months_days*days_hours; % 6 months of follow up
...
%% Main cycle
for hour = 1:(basic_sol+simulation_time)
    
    % If basic solution has been generated, activate parameters leading the
    % competitive cellular events
    if hour > basic_sol
        A(9) = 0.3; % SMC mitosis in medial layer
        A(11) = 0.25; % ECM degeneration in medial layer
    end
    ...
    if severe_stenosis==0,
        ...
        number_of_migration; % show how many cells have migrated!
        ...
        if (mod(hour,1*months_days*days_hours) == 0)
            ...
            % ECM in medial layer
            ECMmedia(ind2) = number_ECM_media;
            ind2 = ind2+1;

            % Medial area
            Med_area(ind4) = medial_area;
            ind3=ind3+1;

            % Graft area
            Graft_area(ind4) = graft_area;
            ind4=ind4+1;
...
% plot the cross section of the graft
graph2
