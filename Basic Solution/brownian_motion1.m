%% Brownian motion1: pressure increased at adventitia interface and relaxed at lumen
...
for kcell=1:Nx*Ny,
    ...
    if type_cell(j,k)>0, % process a site occupied by an SMC or ECM
        ...
        for n=1:6,
            ...
            % if (j3,k3) is empty,and belongs either to lumen or ext support, record its position placing a 1 in the
            ...
            if type_cell(j3,k3)==0 && lumen_hole(j3,k3)+external_support(j3,k3)~=0,
                ...
                for n2=1:6,
                    ...
                    % if (j4,k4) is empty and belongs to lumen or ext
                    % support
        ...
        %% Pressure increaesed at adventitia interface
        for n=1:6,
            if liste(n,1)==0, % remove candidate from the list that have no empty neighbors:
                liste(n,3)=Nx*Ny; % random high number
            end
        end
        % nvide = #empty neighbors of (j,k)
        nvide=sum(liste(:,1));
        
        % Only needed with more than one empty neighbor
        if nvide>1, 
            if lamina(j,k) > 0, % if (j,k) belongs to media --> relaxation ad aventitia interface
                
                % nvide3 --> minimum  (j3,k3)/center CA distance
                % n3 --> index of the closest neighbor of (j,k) to CA's
                % center
                [nvide3,n3] = min(liste(:,3));
                free_ride_cell;
       ...
       %% Pressure relaxed at lumen interface
       for n=1:6,
            if liste(n,1)==0, % remove candidate from the list that have no empty neighbors:
       ...
       % again only needed with more than one empty neighbor
       if nvide>1,
           if lamina(j,k) <= 0, % if (j,k) belongs to intima, we use a pressure relaxation technique

                   % nvide3 --> minimum  (j3,k3)/center CA distance
                   % n3 --> index of the closest neighbor of (j,k) to CA's
                   % center
                   [nvide3,n3] = max(liste(:,3));
                   if nvide3 > 0,
                   free_ride_cell;
                   end
