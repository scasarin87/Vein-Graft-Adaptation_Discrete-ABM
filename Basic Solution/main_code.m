...
basic_sol = 2 * months_days*days_hours; % 3 months of basic sol generation
simulation_time = 8 * months_days*days_hours; % 7 months of follow up

% SMC/ECM vectors initialization
display('Initial distribution of SMC/ECM');
...
%% Main cycle
for hour=1:basic_sol,
    %for hour=1:(basic_sol+simulation_time),
        
%     % If basic solution has been generated, activate paramters leading the
%     % cellular events
%     if hour > basic_sol
%         A(6) = 0.2;
%     end
...
%         % Define the modulation coefficient for p_degeneration
%         xxx_intima = number_ECM_intima/number_SMC_intima;
%         xxx_media = number_ECM_media/number_SMC_media;
        ...
        if (mod(hour,1*months_days*days_hours) == 0)
          %if (mod(hour,24)==0)
            ...
            ECMintima(ind2) = number_ECM_intima;
            ind2 = ind2+1;
            
            SMCmedia(ind3) = number_SMC_media;
            ind3 = ind3+1;
...
graph
% saveas(gcf,'graph2b','fig');

% Plot SMC/ECM in intima/media
all_the_plots
% saveas(gcf,'SmcEcm2','fig');
